...
const Magnetic = new (function() {
    ...
    function A() {
      //console.log(qu);
        ...
        for (var index =0; index<qu.length;index++)
        {
          ...
          if (year<2021)
            value = ((json[year.toFixed(0)][countries[index]]['C']/1000000000)+1).toFixed(0)

          else {
            var value = ((json[2021][countries[index]]['C']/1000000000)+1).toFixed(0)
          }

          const colorVal = Math.floor(Math.random()*value)
          let color = "#bee2e2"
          const me = ((json[2021][countries[index]]['M']/1000000000)+1).toFixed(0)
          const no = ((json[2021][countries[index]]['N']/1000000000)+1).toFixed(0)
          //console.log(((json[2021][countries[index]]['C']/1000000000)+1).toFixed(0),me,no);
          if (colorVal<(value-me-no)) {
            color = "#fbfb80"
          }
          else if (colorVal<(value-me)) {
            color = "#daabda"
          }
    ...
    function P() {
          ...
          if (year<2120)
          {
            year += 0.111; // Increment by 1/30th every time
            ...
            const smallValue = value.toFixed(3);
